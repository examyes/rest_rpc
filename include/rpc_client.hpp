	...
	public:
            ...
            async_read_head([this](const boost::system::error_code& ec, const size_t length) {
				...
				if (!ec) {
					...
					if (body_len == 0 || body_len > MAX_BUF_LEN) {
                        ...
                        close();
                                                error_callback(boost::asio::error::make_error_code(boost::asio::error::message_size));
						return;
					}
				}
				else {
		...
		void read_body(std::uint64_t req_id, request_type req_type, size_t body_len) {
            async_read(body_len,
				...
				if (!socket_.is_open()) {
					//LOG(INFO) << "socket already closed";
                                        call_back(req_id, boost::asio::error::make_error_code(boost::asio::error::connection_aborted), {});
					return;
				}
        ...
        void reset_socket(){
            ...
            if(!socket_.is_open()){
                ...
                class call_t : boost::asio::noncopyable, public std::enable_shared_from_this<call_t> {
		public:
                        call_t(boost::asio::io_service& ios, std::function<void(boost::system::error_code, string_view)> cb, size_t timeout) : timer_(ios),
